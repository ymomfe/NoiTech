const omni_bridge_conduit = extendContent(LiquidExtendingBridge, "omni-bridge-conduit", {
    ...
    acceptLiquid(tile, source, liquid, amount) {  
        ...
        if(tile.getTeam() != source.getTeam() || !this.hasLiquids)
            return false;
        
        if(this.linkValid(tile, other))
            return tile.entity.liquids.get(liquid) + amount < this.liquidCapacity && (tile.entity.liquids.current() == liquid || tile.entity.liquids.get(tile.entity.liquids.current()) < 0.2);
